...
import { Save, Trash, Search, Package, Plus, Video, ChevronDown, ChevronUp } from 'lucide-react';
...
export const KitBuilder: React.FC = () => {
  ...
  const [kitBrand, setKitBrand] = useState<string>('');
  const [kitGender, setKitGender] = useState<GenderType>('unisex');
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if(brandsData.length > 0) setKitBrand(brandsData[0].id);
      setLoading(false);
    };
    loadData();
  }, []);
  ...
  const handleSaveKit = async () => {
    ...
    try {
      ...
      const newKit: Omit<Kit, 'id' | 'createdAt'> = {
        name: kitName,
        ...
        material: 'Algodão', // Default material
        active: true
      };

      await api.createKit(newKit);
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex flex-col gap-6">
      <header className="flex justify-between items-center">
        <div>
           <h2 className="text-2xl font-bold text-primary">Montador de Kits</h2>
           <p className="text-gray-500 text-sm">Crie combos exclusivos com cálculo automático de lucro.</p>
        </div>
        <Button onClick={handleSaveKit} className="gap-2">
          <Save size={18} /> Salvar Kit no Catálogo
        </Button>
      </header>
                ...
                <div key={p.id} className="flex items-center gap-4 p-3 border border-gray-100 rounded-lg hover:bg-blue-50 transition-colors group">
                   ...
                   <div className="flex-1">
                      ...
                      <div className="flex gap-2 text-xs text-gray-500">
                        ...
                        <span>•</span>
                        <span className="font-mono text-gray-700">Custo: R$ {p.costPrice?.toFixed(2) || '0.00'}</span>
                      </div>
                   </div>
                   <button 
                     onClick={() => addToKit(p)}
           ...
           <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                     <label className="block text-sm font-medium text-text-secondary mb-1">Nome do Kit *</label>
                     <input 
                        className="w-full px-4 py-2 border border-gray-300 rounded outline-none focus:border-primary bg-white text-gray-900"
                        placeholder="Ex: Kit Verão 2025 - 10 Peças"
                        value={kitName}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-text-secondary mb-1">Linha</label>
                    <select 
                      className="w-full px-4 py-2 border border-gray-300 rounded outline-none focus:border-primary bg-white text-gray-900"
                      value={kitBrand}
                ...
                <div className="p-4 space-y-4">
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-1">
                            <span className="text-sm text-gray-500">Custo Total dos Itens</span>
                        ...
                        >
                            {showCostDetails ? 'Ocultar detalhes' : 'Ver detalhamento'}
                            {showCostDetails ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
                        </button>
                        
                        {showCostDetails && (
                            <div className="mt-2 bg-gray-50 p-2 rounded text-xs space-y-1 max-h-32 overflow-y-auto border border-gray-200">
                                {kitItems.map((item, idx) => {
                                    ...
                                    return (
                                        <div key={idx} className="flex justify-between">
                                            ...
                                            <span>R$ {(cost * item.quantity).toFixed(2)}</span>
                                        </div>
                                    );
                                })}
                            </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 border-t border-gray-100 pt-4">
                        <div>
                            <label className="block text-xs font-medium text-gray-500 mb-1">Margem Desejada (%)</label>
                            <input 
                                type="number"
                        ...
                        <div>
                            <label className="block text-xs font-medium text-gray-500 mb-1">Preço Sugerido</label>
                            <div className="flex gap-2">
                                <div className="flex-1 px-3 py-2 bg-gray-100 border border-gray-200 rounded text-gray-500 cursor-not-allowed">
                    ...
                    <div className="border-t border-gray-100 pt-4">
                        <label className="block text-sm font-bold text-primary mb-1">Preço de Venda Final (R$) *</label>
                        <input 
                            type="number"
                            className="w-full px-4 py-3 border border-gray-300 rounded-lg outline-none focus:border-primary bg-white text-gray-900 text-lg font-bold"
                            value={kitPrice}
                        ...
                        <div className="flex justify-between items-center mt-2 text-sm">
                            <span className={`px-2 py-1 rounded font-bold ${getMarginColor(currentMargin)}`}>
                                Margem Real: {currentMargin.toFixed(1)}%
                            </span>
              ...
              <div>
                  ...
                  <textarea 
                    className="w-full px-4 py-2 border border-gray-300 rounded outline-none focus:border-primary h-20 resize-none bg-white text-gray-900"
                    placeholder="Descreva o conteúdo e diferenciais do kit..."
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                 <h3 className="font-bold text-primary mb-4 flex items-center gap-2">
                    <Package size={18} /> Itens do Kit
                    <span className="bg-primary text-white text-xs px-2 py-1 rounded-full">{kitItems.length}</span>
                 </h3>
                 ...
                 {kitItems.length === 0 ? (
                    ...
                    <div className="space-y-2">
                         ...
                         <div key={idx} className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-4">
                               <div className="flex items-center border border-gray-200 rounded">
                                  <button onClick={() => updateQuantity(item.productId, -1)} className="px-2 py-1 hover:bg-gray-100">-</button>
                       ...
                       <div className="mt-4 flex justify-between items-center text-sm font-bold text-primary border-t border-gray-200 pt-2">
                          <span>Total de Peças:</span>
                          <span>{kitItems.reduce((a,b) => a + b.quantity, 0)} unid.</span>
                       </div>
                    </div>
                 )}
              </div>
